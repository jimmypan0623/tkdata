...
**********************************開始搜尋指定範圍之發票       
      PROCEDURE CMND1.CLICK      
        ...
        ELSE 
          ...
          ENDIF
          SET ORDER TO K391
          IF RECCOUNT()=0 AND IIF(A02.F05='*',.T.,.F.) &&從這邊決定是自行輸入或是抓字軌檔
             IF INT_IVY   &&以是否需要自行印發票的框線來判斷是自行輸入字軌號碼
             ...
             ELSE &&不需要印線則抓字軌檔
                ****************************************************************************自動抓字軌檔產生          
                ...
                ENDIF
                SET ORDER TO 1
                SEEK  LEFT(THISFORM.MTH_LIST.DISPLAYVALUE,4)
                IF FOUND()      
                   MTH_NO=RIGHT(THISFORM.MTH_LIST.DISPLAYVALUE,2)
                   SCNT=1
                   STD_NO=K36.F02
                   END_NO=K36.F03
                   JILAN='F'+PADL(ALLTRIM(STR(3+(VAL(MTH_NO)+1)/2)),2,'0')
                   JIQUE=K36.&JILAN
                   DO WHILE STD_NO<=K36.F03
                      SELECT K37
                      APPEND BLANK
                      REPLACE F01 WITH PADL(ALLTRIM(STR(SCNT)),4,'0')
                      REPLACE F02 WITH '31'
                      REPLACE F03 WITH JIQUE+STD_NO
                      REPLACE F06 WITH F03
                      FOR I=1 TO 50
                          SELECT K39
                          APPEND BLANK
                          REPLACE F01 WITH K37.F01
                          REPLACE F02 WITH JIQUE+STD_NO
                          STD_NO=PADL(ALLTRIM(STR(VAL(STD_NO)+1)),8,'0')
                      ENDFOR
                      SELECT K37
                      REPLACE F04 WITH JIQUE+PADL(ALLTRIM(STR(VAL(STD_NO)-1)),8,'0')
                      REPLACE F07 WITH 50
                      SCNT=SCNT+1
                   ENDDO
                   SELECT K36
                   REPLACE F11 WITH MTH_NO
                   USE
                   CT=INT((SCNT-1)/2)
                   SELECT K37 
                   ...
                   K2Kform.show                                              
                ELSE
                   =MESSAGEBOX(LEFT(THISFORM.MTH_LIST.DISPLAYVALUE,4)+'年度之電子計算機號碼與字軌尚未設定!',0+48,'提示訊息視窗')
                   SELECT K36
                   USE
                   THISFORM.CMND2.CLICK
                ENDIF
...
define class tkK2K as form
  ...
  ADD OBJECT RTG_BOTT AS COMMANDBUTTON WITH;
      ...
      CAPTION='\<T.編輯人員',;
      NAME='RTG_BOTT'                       
  ADD OBJECT RLP_BOTT AS COMMANDBUTTON WITH;
      TOP=INT_015*460,;
      LEFT=INT_015*84,;
      HEIGHT=INT_015*25,;
      FONTSIZE=INT_015*9,;
  ...
  PROCEDURE PAGEFRAME1.PAGE1.activate
    ...
    IF THISFORM.GRID1.ACTIVEROW=0 .AND. thisform.SHRGROUP.ENABLED=.F. 
     ...
     ELSE
        ...
        THISFORM.EXL_BOTT.ENABLED=IIF(A02.F09='*',.T.,.F.) AND (MONTH(DATE())>MONTH(CTOD(HKS+'/01')) OR YEAR(DATE())>YEAR(CTOD(HKS+'/01')))
        THISFORM.EPT_BOTT.ENABLED=IIF(A02.F10='*',.T.,.F.) AND (MONTH(DATE())>MONTH(CTOD(HKS+'/01')) OR YEAR(DATE())>YEAR(CTOD(HKS+'/01')))
     ENDIF   
      THISFORM.ORPGROUP.EDIT_BOTT.VISIBLE=.T.
      THISFORM.RLP_BOTT.VISIBLE=.F.
...
*       thisform.setall('height',INT_015*17,'label')
       ...
       THISFORM.grid2.COLUMN3.HEADER1.CAPTION='列印日期'       
       THISFORM.grid2.COLUMN4.HEADER1.CAPTION='解除列印限制說明'                    
	   ...
	   THISFORM.grid2.COLUMN3.CONTROLSOURCE='K39.F05'	
	   THISFORM.grid2.COLUMN3.WIDTH=INT_015*70
	   THISFORM.grid2.COLUMN4.CONTROLSOURCE='K39.F06'	
	   THISFORM.grid2.COLUMN4.WIDTH=INT_015*270
       THISFORM.GRID2.SETALL("DYNAMICBACKCOLOR","IIF(K39.F04='-',RGB(255,0,0),IIF(K39.F04='+',RGB(255,255,0),''))","COLUMN")	 
...
*       thisform.refresh
  ...
  PROC grid2.AFTERROWCOLCHANGE
       ...
       THISFORM.RLP_BOTT.ENABLED=!EMPTY(F05) AND F04<>"+" AND IIF(A02.F11='*',.T.,.F.) 
       FCH=THISFORM.ACTIVECONTROL.NAME
       JK=THIS.COLUMN1.HEADER1.CAPTION
   ...
   ENDPROC
    ...
    PROCEDURE EXL_BOTT.CLICK    &&轉申報檔
          ...
          **********************開檔案
          ...
          SET ORDER TO 1
          CREATE CURSOR SALE;
          (F01 C(5),F02 C(8),F03 C(5),F04 C(1),F05 C(4),F06 C(10),F07 C(8),F08 C(12),F09 C(5),F10 C(12),F11 C(4),F12 C(4),F13 C(10),F14 C(15),F15 C(10),F16 C(1),F17 C(20),F18 C(1),F99 C(2),FT1 C(10))
          ...
          DO WHILE !EOF()
             ...
             APPEND BLANK
                    ...
                    REPLACE F07 WITH IIF(SEEK(F06,'K251'),ICASE(K251.F16='2','+',K251.F16='4','-',K251.F04),IIF(SEEK(F06,'K252'),ICASE(K252.F16='2','+',K252.F16='4','-',K252.F04),'-'))  &&修改這裡
                    ...
                    ENDIF                    
                    REPLACE F16 WITH 'N'                                
                    REPLACE FT1 WITH IIF(LEFT(F07,1)='-','',F06 )      
          ...
          ENDDO          
          &&產生空白發票檔
          CREATE CURSOR EMPTYINV (F01 C(5),F02 C(8),F03 C(5),F04 C(2),F05 C(8),F06 C(8),F07 C(2))
          TMPCNT=0
          SELECT SALE
          SET ORDER TO 1
          GO TOP
          DO WHILE !EOF()
             IF LEFT(F07,1)$ "*-"
                ...
                SELECT EMPTYINV
                APPEND BLANK
                REPLACE F01 WITH PADL(ALLTRIM(STR(TMPCNT)),5,'0')
                REPLACE F02 WITH INT_107
                REPLACE F03 WITH PADL(ALLTRIM(STR(VAL(LEFT(TXMTH2,4))-1911)),3,'0')+RIGHT(TXMTH2,2)
                REPLACE F04 WITH LEFT(SALE.F06,2)
                REPLACE F05 WITH RIGHT(SALE.F06,8)
                SELECT SALE         
                IF LEFT(SALE.F07,1)="-"                                      
                   DO WHILE LEFT(F07,1)='-'
                      SKIP
                ...
                ELSE                            
                   ...
                   IF !EMPTY(F13)
                      SKIP -1
                   ELSE
                      DO WHILE EMPTY(F13) AND RIGHT(F06,2)<>'00' AND RIGHT(F06,2)<>'50' AND !EOF() 
                         SKIP
                      ENDDO  
                      SKIP -1
                   ENDIF   
                ENDIF               
                SELECT EMPTYINV
                REPLACE F06 WITH RIGHT(SALE.F06,8)
                REPLACE F07 WITH '07'  
            ENDIF  
            SELECT SALE
            SKIP       
          ENDDO
          SELECT EMPTYINV
           FILE_NAME='E0501_'+INT_107+'_'+JST
             GCDELIMFILE=PUTFILE('儲存路徑',FILE_NAME,'CSV')
             IF !EMPTY(GCDELIMFILE)  && ESC PRESSED
                 COPY TO (GCDELIMFILE) DELIMITED WITH "" WITH CHARACTER ","
          ...
          *****往下加出口虛擬發票
           ZEA=1
          FOR I=1 TO IIF(LEFT(DTOS(DATE()),6)>TXMTH2,2,1)
              INVTB='K25'+ALLTRIM(STR(I))
              SELECT * FROM &INVTB WHERE LEFT(F07,2)$'6769' AND F01='32' INTO CURSOR K25A ORDER BY F07,F15 NOWAIT
              IF _TALLY>0                                  
                 ...
                 DO WHILE !EOF()
                    ...
                    REPLACE F01 WITH JST
                    REPLACE F02 WITH 'TOKUTSU'     
                    REPLACE F03 WITH SUBSTR(K25A.F19,2,3)+RIGHT(K25A.F19,2)                 
                    REPLACE F04 WITH '5'            
                    REPLACE F05 WITH PADL(ALLTRIM(STR(ZEA)),4,'0') 
                    REPLACE FT1 WITH K25A.F07
                    REPLACE F06 WITH LEFT(FT1,6)+PADL(ALLTRIM(STR(CTT)),4,'0')
                    REPLACE F99 WITH K25A.F01
                    REPLACE F08 WITH PADL(ALLTRIM(STR(K25A.F12)),12,'0')
                    REPLACE F09 WITH '01'
                    REPLACE F10 WITH PADL('0',12,'0')
                    REPLACE F11 WITH RIGHT(K25A.F19,2)+K25A.F02 
                    REPLACE F13 WITH K25A.F03
                    REPLACE F16 WITH 'N'
                    REPLACE F17 WITH K25A.F06     
                    REPLACE F18 WITH K25A.F05 
 *                   REPLACE F03 WITH CTOD(PADL(LEFT(SALE.F03,3),4,'0')+'/'+LEFT(SALE.F11,2)+'/'+RIGHT(SALE.F11,2))
                   
	                SELE K25A
                    SKIP                       
                    IF RIGHT(STR(CTT),2)='49' OR RIGHT(STR(CTT),2)='99'
                       ZEA=ZEA+1          
                    ENDIF
                    IF ZG<>LEFT(K25A.F07,2)
                       TMPNO=SALE.F06
                       CTT=CTT+1
                       SELECT SALE
                       APPEND BLANK
                       ...
                       REPLACE F04 WITH'5'
                       REPLACE F05 WITH PADL(ALLTRIM(STR(ZEA)),4,'0')
                       REPLACE F06 WITH LEFT(TMPNO,6)+PADL(ALLTRIM(STR(CTT)),4,'0')
                       REPLACE F07 WITH '*'
                       REPLACE F08 WITH PADL('0',12,'0')
                       REPLACE F10 WITH PADL('0',12,'0')
                       REPLACE F16 WITH 'N'
                       
                       &&REPLACE FT1 WITH LEFT(TMPNO,6)+PADL(ALLTRIM(STR(CTT)),4,'0')
                       DO WHILE .T. 
                          IF RIGHT(F06,2)='49' OR RIGHT(F06,2)='99'
                            EXIT
                          ENDIF  
                          CTT=CTT+1
                          APPEND BLANK
              ...
              *****                
           ENDFOR
          SELECT F01 送件年月,F02 用戶代號,PADL(ALLTRIM(STR(VAL(LEFT(TXMTH2,4))-1911)),3,'0')+RIGHT(TXMTH2,2) 所屬年月,F04 發票聯式,F05 第冊數,F06 發票號碼,IIF(LEFT(F07,1)='+','+'+SPACE(7),F07) 統一編號,F08 銷售額,F09 稅別,F10 稅額,F11 月日,F12 營業稅科目類別代號,F13 客戶代號,F14 公斤數,F18 通關註記,0 免稅銷售額,F15+F16 出售註記及列印註記,FT1 原始發票 FROM SALE NOWAIT
          IF _TALLY>0
          ...
          '電子連接器等'+SPACE(4) 貨物勞務,PADL(ALLTRIM(STR(TTL2(FT1,F03))),12,'0') 數量,SUBSTR(F09,2,1) 外銷方式,SPACE(12) 備註,IIF(F99='31','發票'+SPACE(8),IIF(F18='1','X7'+SPACE(10),SPACE(12))) 非經海關證明文件名稱,;
          ...
          UPDATE K39 SET F04='+' WHERE F02=ANY(SELE F06 FROM SALE WHERE LEFT(F07,1)='+')
          UPDATE K251 SET K251.F07=SALE.F06 WHERE K251.F07=SALE.FT1 AND RIGHT(K251.F19,2)=LEFT(SALE.F11,2) AND LEFT(K251.F07,2)$'6769' AND K251.F01='32' FROM K251,SALE
          UPDATE K252 SET K252.F07=SALE.F06 WHERE K252.F07=SALE.FT1 AND RIGHT(K252.F19,2)=LEFT(SALE.F11,2) AND LEFT(K252.F07,2)$'6769' AND K252.F01='32' FROM K252,SALE
          UPDATE C131 SET C131.F17=SALE.F06 WHERE C131.F17=SALE.FT1 AND C131.F08>0 AND LEFT(C131.F17,2)='67' FROM C131,SALE
          UPDATE C132 SET C132.F17=SALE.F06 WHERE C132.F17=SALE.FT1 AND C132.F08>0 AND LEFT(C132.F17,2)='67' FROM C132,SALE
          UPDATE B061 SET B061.F20=SALE.F06 WHERE B061.F20=SALE.FT1 AND LEFT(B061.F20,2)='69' FROM B061,SALE
          UPDATE B062 SET B062.F20=SALE.F06 WHERE B062.F20=SALE.FT1 AND LEFT(B062.F20,2)='69' FROM B062,SALE
          UPDATE B041 SET B041.F20=SALE.F06 WHERE B041.F20=SALE.FT1 AND LEFT(B041.F20,2)='67' FROM B041,SALE
          UPDATE B042 SET B042.F20=SALE.F06 WHERE B042.F20=SALE.FT1 AND LEFT(B042.F20,2)='67' FROM B042,SALE
          SELECT SALE
          USE
          SELECT C01
          USE
...
*!*	       ENDIF
       ...
       THISFORM.GRID1.SETFOCUS
     ENDPROC
     PROCEDURE RLP_BOTT.CLICK
       SELECT K39    
       RLPINV=CREATEOBJECT("RLPINV")  
       RLPINV.SHOW           
       THISFORM.GRID2.SETFOCUS 
     ENDPROC
  ENDDEFINE        
...
ADD OBJECT DATA_OPTION AS OPTIONGROUP WITH;
      ...
      FONTSIZE=INT_015*12,;
      BUTTONCOUNT=2,;
      OPTION1.CAPTION='列印全部空白發票',;
      OPTION1.TOP=INT_015*0,;
      OPTION1.FONTSIZE=INT_015*9,;
      OPTION1.AUTOSIZE=.T.,;
      OPTION2.CAPTION='只印單張空白發票',;
  ...
  *****      
 PROC INIT  
     THISFORM.LBL1.VISIBLE=.F.
     THISFORM.TXT1.VISIBLE=.F.
     THISFORM.TXT1.ENABLED=.F.
     THISFORM.TXT2.VISIBLE=.F.
 ...
 PROCEDURE DATA_OPTION.INTERACTIVECHANGE
      ...
      ELSE
         ...
         THISFORM.TXT1.VALUE=LEFT(K39.F02,2)
         THISFORM.TXT2.VISIBLE=.T.
 ...
 PROCEDURE CMND1.CLICK     
     IF THISFORM.DATA_OPTION.VALUE=1
        SELECT F02 FROM K39 WHERE F04='+' INTO CURSOR K39_1 ORDER BY F02 NOWAIT 
     ELSE
        SELECT F02 FROM K39 WHERE F04='+' AND F02=THISFORM.TXT1.VALUE+THISFORM.TXT2.VALUE INTO CURSOR K39_1 ORDER BY F02 NOWAIT 
     ENDIF
     IF _tally >0
          SELECT K39_1
          GO TOP
          ...
          USE 
     ELSE
           =MESSAGEBOX('無此範圍空白發票可列印!或是請先做轉申報檔後再列印!',0+48,'提示訊息視窗')           
     ENDIF  
     THISFORM.RELEASE  
 ENDPROC   
 ***** 
...
DEFINE CLASS RLPINV AS FORM
 ...
 PROC INIT  
     ...
     THISFORM.TXT1.VALUE=K39.F06
     
 ENDPROC  

 PROCEDURE CMND1.CLICK     
     IF EMPTY(THISFORM.TXT1.VALUE)
        =MESSAGEBOX('備註不得空白!',0+48,'提示訊息視窗')      
        THISFORM.TXT1.SETFOCUS
        RETURN   
     ENDIF
     SELECT K39
     REPLACE F05 WITH CTOD('')
     REPLACE F06 WITH THISFORM.TXT1.VALUE
     THISFORM.CMND2.CLICK 
 ENDPROC   
 ***** 
  PROCEDURE CMND2.CLICK
