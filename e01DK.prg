...
ENDIF
SET ORDER TO E011
SET RELATION TO F02 INTO E00 ADDI
*************************建立表單E01FORM*************************         
E01FORM=CREATEOBJECT("TKE01")
E01FORM.SHOW  
DEFINE CLASS TKE01 AS FORM
  CAPTION='E01.工作部門產能設定'
*!*	  AUTOCENTER=.T.
  CONTROLBOX=.F.
  BORDERSTYLE=1
  MAXBUTTON=.F.
...
PROC INIT
       ...
       THISFORM.GRID2.COLUMN1.HEADER1.CAPTION='製程代碼'                                         
       THISFORM.GRID2.COLUMN2.HEADER1.CAPTION='製程說明'
       THISFORM.GRID2.COLUMN3.HEADER1.CAPTION='單位工時'
       THISFORM.GRID2.COLUMN4.HEADER1.CAPTION='閒置工時' 
       THISFORM.GRID2.COLUMN1.CONTROLSOURCE='E01.F02'
       THISFORM.GRID2.COLUMN2.CONTROLSOURCE='E00.F04'
       THISFORM.GRID2.COLUMN3.CONTROLSOURCE="E01.F03"
	   ...
	   THISFORM.ORPGROUP.DEL_BOTT.ENABLED=IIF(A02.F06='*',.T.,.F.) AND IIF(SEEK(A14.F01,'E01'),.T.,.F.)   &&判斷有無刪除的權限  
...
***************GRID1動態改變*********        
  PROC GRID1.AFTERROWCOLCHANGE
       LPARAMETERS NCOLINDEX
          ...
          THISFORM.PAGEFRAME1.PAGE1.TXTF07.VALUE=A14.F07
          THISFORM.PAGEFRAME1.PAGE1.TXTF02.VALUE=BILL(A14.F02)
          THISFORM.PAGEFRAME1.PAGE1.TXTF03.VALUE=A14.F03
          THISFORM.PAGEFRAME1.PAGE1.TXTF11.VALUE=A14.F11
          THISFORM.PAGEFRAME1.PAGE1.TXTF12.VALUE=A14.F12
          THISFORM.PAGEFRAME1.PAGE1.LBLF131.CAPTION=A14.F13
          THISFORM.ORPGROUP.DEL_BOTT.ENABLED=IIF(A02.F06='*',.T.,.F.) AND IIF(SEEK(A14.F01,'E01'),.T.,.F.)   &&判斷有無刪除的權限    
...
***************GRID2動態改變***************       
  PROC GRID2.AFTERROWCOLCHANGE
       LPARAMETERS NCOLINDEX        
       SELECT E01            
       THISFORM.PAGEFRAME1.PAGE2.TXTF02.VALUE=E01.F02
       THISFORM.PAGEFRAME1.PAGE2.TXTF03.VALUE=E01.F03
  ...
  IF THISFORM.PAGEFRAME1.ACTIVEPAGE=2  &&表身確認
       ...
       IF (ASC(LEFT(THISFORM.PAGEFRAME1.PAGE2.TXTF02.VALUE,1))>=65 AND ASC(LEFT(THISFORM.PAGEFRAME1.PAGE2.TXTF02.VALUE,1))<=95)=.F.
           IF (ASC(LEFT(THISFORM.PAGEFRAME1.PAGE2.TXTF02.VALUE,1))>=97 AND ASC(LEFT(THISFORM.PAGEFRAME1.PAGE2.TXTF02.VALUE,1))<=122)=.F.   
                =MESSAGEBOX('製程代號1碼須為英文字母!',0+64,'提示視窗')
                THISFORM.PAGEFRAME1.PAGE2.TXTF02.SETFOCUS
                SET ORDER TO &COD
                RETURN
           ENDIF 
       ENDIF  
       IF A14.F02='1' AND  LEN(ALLTRIM(THISFORM.PAGEFRAME1.PAGE2.TXTF02.VALUE))>5
     	    =MESSAGEBOX('此資料檔之檔案類別為月份檔,故製程代號不得超過5各字元,請重新輸入!',0+64,'提示視窗')
            THISFORM.PAGEFRAME1.PAGE2.TXTF02.SETFOCUS
            RETURN    
            SET ORDER TO &COD          
       ENDIF
       IF  IIF(SEEK(ALLTRIM(THISFORM.PAGEFRAME1.PAGE1.TXTF01.VALUE)+SPACE(8-LEN(ALLTRIM(THISFORM.PAGEFRAME1.PAGE1.TXTF01.VALUE)))+THISFORM.PAGEFRAME1.PAGE2.TXTF02.VALUE,'E01'),.T.,.F.)
     	    =MESSAGEBOX('此製程代號已存在,請重新輸入!',0+64,'提示視窗')
            THISFORM.PAGEFRAME1.PAGE2.TXTF02.SETFOCUS
            RETURN    
            SET ORDER TO &COD   
       ENDIF
...
PROCEDURE ORPGROUP.DEL_BOTT.CLICK     
   IF THISFORM.PAGEFRAME1.ACTIVEPAGE=1
        IF MESSAGEBOX('是否確定要刪除此筆資料檔之所有製程代號資料',4+32+256,'請確認')=6
             SELECT  RECNO() FROM E01 WHERE ALLTRIM(E01.F01)=ALLTRIM(THISFORM.PAGEFRAME1.PAGE1.TXTF01.VALUE) INTO ARRAY BK1              
             JJ1=''                
             IF _TALLY>0   
                 FOR I= 1 TO ALEN(BK1)
             ...
             IF LL1 AND LEN(ALLTRIM(JJ1))>0 
                 ...
                 FOR I= 1 TO ALEN(BK1)
                  ...
                  ENDFOR
              ENDIF 
              =MESSAGEBOX('此筆資料檔之所有製程代號資料已刪除成功!',0+64,'提示視窗')
              RELEASE ALL LIKE BK*         
              THISFORM.REFRESH                    
              THISFORM.SHRGROUP.ABANDON_BOTT.CLICK
        ENDIF  
   ELSE
        IF MESSAGEBOX('是否確定要刪除此製程代號之資料',4+32+256,'請確認')=6  
             SELECT  E01  
             LOCK()
             IF  RLOCK()
                  DELETE 
		  ...
		  IF THISFORM.GRID2.ACTIVEROW=0 
		   ...
		   ELSE
		       ...
		       THISFORM.ORPGROUP.DEL_BOTT.ENABLED=IIF(A02.F06='*',.T.,.F.)   &&判斷有無刪除的權限
		    ENDIF 
		     =MESSAGEBOX('此筆製程代號之資料已刪除成功!',0+64,'提示視窗')
		    THISFORM.SHRGROUP.ABANDON_BOTT.CLICK 
             ELSE
                    DO FILE_IMPACT 
                    RETURN    
 ...
 ***********************************************列印程序
 PROCEDURE PNT_PRC  
       PO_NO=A14.F01
*!*	       SELECT  ORDER BY A22.F01 FROM A14,A22 WHERE A22.F05=PO_NO AND A14.F01=PO_NO NOWAIT
*!*	       REPORT FORM ALLTRIM(INT_116)+'E01' TO PRINT PROMPT PREVIEW
*!*	       CLEAR
 ENDPROC
********************
FUNC BILL
