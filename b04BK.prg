...
CREATE CURSOR B04_1;
(F01 C(10),F02 C(2),F06 C(5),F08 C(1),F09 C(5),F10 C(1),F11 C(17),F20 C(10),F21 C(60),F22 C(2),F23 C(1),F24 C(40),F90 C(17))
INDE ON F01 TAG B04_11
INDE ON F06+F01 TAG B04_12
SELE F01,F02,F06,F08,F09,F10,F11,F20,F21,F22,F23,F24,F90 FROM B04 GROUP BY F01 ORDER BY F01 INTO CURSOR B04_2
SELE B04_2
GO TOP
DO WHILE !EOF()
   SELE B04_1
...
*!*	      AUTOSIZE=.T.      
 ...
 PROC INIT       
       ...
       THISFORM.GRID1.COLUMN3.CONTROLSOURCE="IIF(SEEK(B04_1.F06,'C01'),C01.F05,'')"
  ...
  PROC KEY_LIST.INTERACTIVECHANGE                 
       ...
       DO CASE
          ...
          CASE THIS.DISPLAYVALUE='依客戶編號排列'
               ...
               THISFORM.GRID1.COLUMN2.CONTROLSOURCE="IIF(SEEK(B04_1.F06,'C01'),C01.F05,'')"
...
*       SET RELATION TO F05 INTO A14
       CREATE CURSOR B04_1;
       (F01 C(10),F02 C(2),F06 C(5),F08 C(1),F09 C(5),F10 C(1),F11 C(17),F20 C(10),F21 C(60),F22 C(2),F23 C(1),F24 C(24),F90 C(17))
       INDE ON F01 TAG B04_11
       INDE ON F06+F01 TAG B04_12
       SELE F01,F02,F06,F08,F09,F10,F11,F20,F21,F22,F23,F24,F90 FROM B04 GROUP BY F01 ORDER BY F01 INTO CURSOR B04_2
       SELE B04_2
...
*       THISFORM.PAGEFRAME1.PAGE2.LBLF051.CAPTION=IIF(SEEK(B04.F05,'A14'),A14.F02,'')       
...
*       IF INT_099=.T.
          H='B003 '+SUBSTR(DTOS(CTOD(THISFORM.MTH_LIST.DISPLAYVALUE+'/01')),3,4)
          SELE A24   
          SET ORDER TO 1
          SEEK H           
          IF FOUND() 
     ...
     ELSE
        ...
        THISFORM.PAGEFRAME1.PAGE2.LBLF051.CAPTION=IIF(SEEK(INT_193,'A14'),A14.F02,INT_193)              
  ...
  PROCEDURE ORPGROUP.EDIT_BOTT.CLICK
     ...
     ELSE           
        ...
        SELE B04                                                      &&處理表身修改
        SELE RECNO() FROM C05 WHERE F01=B04.F07 AND F02=B04.F03 INTO ARRAY BK1
        JJ1=''                
        IF _TALLY>0                   
           FOR I= 1 TO ALEN(BK1)
               JJ1=JJ1+ALLTRIM(STR(BK1(I)))+','
          ...
          IF ALIA()='B04_1'           &&刪除整張訂單
             SELE B04_1             
                ...
                ELSE
                   LL1=.T.   
                ENDIF                   
                SELE RECNO() FROM C05 WHERE F01= ANY (SELE F07 FROM B04 WHERE F01=PO_NO) INTO ARRAY BK2
                JJ2=''                
                IF _TALLY>0                   
                    FOR I= 1 TO ALEN(BK2)
...
*                                REPL F19 WITH F19+(B04.F13)*(-1)
                 ...
                 IF INT_099=.T. .AND. SUBSTR(PO_NO,3,4)=SUBSTR(DTOS(CTOD(THISFORM.MTH_LIST.DISPLAYVALUE+'/01')),3,4)
                      SELE A24
                      SET ORDER TO 1
                      SEEK 'B003 '+SUBSTR(DTOS(CTOD(THISFORM.MTH_LIST.DISPLAYVALUE+'/01')),3,4)+PO_NO
                      IF !FOUND()
                          APPEND BLANK
                          LOCK()
                          REPLACE  F01 WITH 'B003 '
...
*                        REPL F19 WITH F19+(B04.F13)*(-1)
  ...
  PROCEDURE SHRGROUP.SHURE1_BOTT.CLICK               &&新增確認
      IF THISFORM.PAGEFRAME1.ACTIVEPAGE=1
       ...
       COD=SYS(21)
       SET ORDER TO 1
       IF SEEK(THISFORM.PAGEFRAME1.PAGE1.TXTF01.VALUE,'B04_1')=.T.    &&如果單號重覆
             SELE A24
             SEEK 'B003 '+SUBSTR(DTOS(DATE()),3,4)+THISFORM.PAGEFRAME1.PAGE1.TXTF01.VALUE
             IF FOUND()
                 DO WHILE F01+F02+F08='B003 '+SUBSTR(DTOS(DATE()),3,4)+THISFORM.PAGEFRAME1.PAGE1.TXTF01.VALUE
                        RLOCK()
                        DELE
                        UNLOCK
                 ENDDO   
             ENDIF                   
             SELE A09
             SEEK 'B003 '+SUBSTR(DTOS(DATE()),3,4)
             IF FOUND()
                 RLOCK()
		 HX=VAL(RIGHT(F08,4))+1
   		 PO_NO=LEFT(F08,6)+PADL(ALLTRIM(STR(HX)),4,'0')
       ...
       IF RLOCK()
          ...
          IF LEN(ALLTRIM(THISFORM.PAGEFRAME1.PAGE1.TXTF20.VALUE))=10 OR INT_060=.F.     &&如果有發票號碼或預設不帶入發票管理
              ...
              REPLACE  F08 WITH IIF(THISFORM.PAGEFRAME1.PAGE1.CUS_TYPE.VISIBLE=.F.,'',IIF(THISFORM.PAGEFRAME1.PAGE1.CUS_TYPE.VALUE=2,'2','1'))
          ELSE 
             REPLACE  F22 WITH ''
             REPLACE  F23 WITH ''   
             REPLACE  F08 WITH ''
           ...
           IF FOUND()
              DO WHILE F01=B04_1.F01
                 ...
                 IF LEN(ALLTRIM(THISFORM.PAGEFRAME1.PAGE1.TXTF20.VALUE))=10 OR INT_060=.F.   &&如果有發票號碼或預設不帶入發票管理
                    ...
                    REPL F08 WITH IIF(THISFORM.PAGEFRAME1.PAGE1.CUS_TYPE.VISIBLE=.F.,'',IIF(THISFORM.PAGEFRAME1.PAGE1.CUS_TYPE.VALUE=2,'2','1'))
                 ELSE
                    REPL F22 WITH ''
                    REPL F23 WITH ''    
                    REPL F08 WITH ''
...
*        IF RLOCK() 
           ...
           REPL F20 WITH THISFORM.PAGEFRAME1.PAGE1.TXTF20.VALUE
           REPL F21 WITH THISFORM.PAGEFRAME1.PAGE1.TXTF21.VALUE     
           REPL F24 WITH THISFORM.PAGEFRAME1.PAGE1.TXTF24.VALUE                  
           REPL F11 WITH DTOC(DATE())+' '+IIF(SEEK(SYS_OPER,'A01'),A01.F03,'')
           UNLOCK ALL
*        ELSE
*           DO FILE_IMPACT
      ...
      IF THISFORM.PAGEFRAME1.ACTIVEPAGE=1
         IF THISFORM.GRID2.RECORDSOURCE<>'B04'
            ...
            THISFORM.GRID2.CHILDORDER=B041
  	        THISFORM.GRID2.COLUMN1.CONTROLSOURCE='B04.F03'
	        THISFORM.GRID2.COLUMN2.CONTROLSOURCE='B01.F02'
			THISFORM.GRID2.COLUMN3.CONTROLSOURCE="IIF(SEEK(B04.F05,'A14'),A14.F02,B04.F05)"
			THISFORM.GRID2.COLUMN4.CONTROLSOURCE='B04.F07'
			THISFORM.GRID2.COLUMN5.CONTROLSOURCE='B04.F04'
			THISFORM.GRID2.COLUMN6.CONTROLSOURCE='B04.F13'
			THISFORM.GRID2.COLUMN7.CONTROLSOURCE='B04.F12'
       ...
       IF MESSAGEBOX('是否確認資料無誤可以過帳?',4+32+256,'請確認') = 6	        
          ...
          SELE RECNO() FROM B04 WHERE B04.F01=B04_1.F01  AND F10<>'2' INTO ARRAY BK1              
               ...
               ELSE
                  LL1=.T.   
               ENDIF                   
               SELE RECNO() FROM C05 WHERE F01= ANY (SELE F07 FROM B04 WHERE F01=B04_1.F01) INTO ARRAY BK2
               JJ2=''                
               IF _TALLY>0                   
                  FOR I= 1 TO ALEN(BK2)
                      JJ2=JJ2+ALLTRIM(STR(BK2(I)))+','
               ...
               ELSE
                  LL3=.T.   
               ENDIF                   
               SELE RECNO() FROM C03 WHERE F02=ANY (SELE F07 FROM B04 WHERE F01=B04_1.F01) INTO ARRAY BK4
               JJ4=''
               IF _TALLY>0   
                  FOR I= 1 TO ALEN(BK4)
               ...
               IF LL1 .AND. LL1 .AND. LL2 .AND. LL3 .AND. LL4 .AND. LL5=.T. 
                  IF LEN(ALLTRIM(JJ1))>0 
                     ...
                     FOR I= 1 TO ALEN(BK1)                      
                         ...
                         IF FOUND()                                          &&物料基本主檔存在才做運算    
                            IF !EMPTY(F98)                                   &&如果不是虛擬料號才做庫存運算                            
                               SELE B11                                            &&部門庫存明細檔 
                               SEEK B04.F05+B04.F03+IIF(INT_028 AND B01.F98<>'A  ',B04.F06,SPACE(5))
                               IF FOUND()
                         ...
                         REPL F15 WITH IIF(SEEK(B04.F07,'C03'),'訂單:'+C03.F14,'')+IIF(SEEK(B04.F07+B04.F03,'C04'),'品號:'+C04.F17,'')+IIF(LEN(ALLTRIM(B04.F20))=10,'發票:'+B04.F20,'')
                         ...
                         IF IIF(SEEK(B04.F03,'B01'),B01.F97,'N') = 'Y' AND B04.F13 <> 0 &&紀錄樣品線材
                            SELECT C39	
                            APPEND BLANK
                    ...
                    IF LEN(ALLTRIM(B04_1.F20))=10  AND  INT_060=.T. &&如果有發票號碼且要帶入發票管理
                       ...
                       ENDIF                          
                       APPEND BLANK
                       REPL F01 WITH B04_1.F22
                       IF B04_1.F02<=IIF(SEEK(B04_1.F06,'C01'),C01.F17,'')
                          REPL F02 WITH B04.F02
                          REPL F19 WITH THISFORM.MTH_LIST.DISPLAYVALUE 
                       ELSE
                          REPL F02 WITH '01'
                          REPL F19 WITH LEFT(DTOC(GOMONTH(CTOD(THISFORM.MTH_LIST.DISPLAYVALUE+'/'+'01'),1)),5)
                       ENDIF                                                                  
                       REPL F03 WITH B04_1.F06                                &&客戶編號
                       REPL F04 WITH IIF(SEEK(B04.F06,'C01'),C01.F10,'')      &&統一編號
                       REPL F05 WITH B04_1.F08                                &&是否經海關->' ' 不出口 '1'不經海關  '2'經海關     
                       IF F05='2' 
                          REPL F06 WITH B04_1.F21                             &&報單號碼
                       ENDIF   
                       REPL F07 WITH B04_1.F20                                &&發票號碼
                       REPL F09 WITH B04_1.F23                                &&課稅別(1-應 2-零  3-免)                          
                       IF F01='31'                                            		     &&若為銷項三聯式. 電子計算機
                          REPL F08 WITH TQTY         			     &&銷售金額 >> 小計
                          REPL F10 WITH ROUND(TQTY*INT_002/100,INT_069)*IIF(F09='1',1,0) &&營業稅額=ROUND(小計*採購訂單稅率,應收對帳單四捨五入)
                          REPL F12 WITH F08+F10				     &&發票總額
                       ELSE
                          REPL F12 WITH TQTY
                          REPL F08 WITH ROUND(TQTY/IIF(F09='1',1+(INT_002/100),1),INT_069)
                          REPL F10 WITH F12-F08
                       ENDIF   
                       REPL F13 WITH '00'
                       REPL F15 WITH B04_1.F01+IIF(B01.F97='Y','製品','商品')
                       REPL F16 WITH '1'
                       REPL F18 WITH '01'
                       REPL F20 WITH ALLTRIM(B04_1.F24)
                       REPL F21 WITH B04.F14
                       REPL F22 WITH B04.F16
                       REPL F23 WITH FTTL
...
********************
  PROC VRT_BOTT.CLICK     &&反過帳程序  
       IF MESSAGEBOX('是否確認必須反過帳以修正單據內容?',4+32+256,'請確認') = 6	        
           SELECT B04.F04+B04.F13, C04.F05-C04.F10-C04.F21-C04.F24 FROM B04,C04 WHERE B04.F01=THISFORM.PAGEFRAME1.PAGE1.TXTF01.VALUE AND C04.F03+C04.F04=B04.F07+B04.F03 AND B04.F04+B04.F13>C04.F05-C04.F10-C04.F21-C04.F24 NOWAIT
           IF _TALLY>0
               =MESSAGEBOX('退貨數量已大於可以反過帳之數量!此項作業不得執行!',0+48,'提示訊息視窗')
               IF THISFORM.PAGEFRAME1.ACTIVEPAGE=1
                    THISFORM.GRID1.SETFOCUS
                ELSE
                    THISFORM.GRID2.SETFOCUS
               ...
               IF LL1 .AND. LL1 .AND. LL2 .AND. LL3 .AND. LL4 .AND. LL5=.T. 
                  IF LEN(ALLTRIM(JJ1))>0 
                     ...
                     FOR I= 1 TO ALEN(BK1)                      
                         ...
                         SELE C05                                            &&出貨計劃
                         SET ORDER TO C054                 
                         SEEK B04.F07+B04.F03+DTOS(CTOD(THISFORM.MTH_LIST.DISPLAYVALUE+'/'+ B04.F02))
                         IF FOUND()
                         ...
                         REPL F03 WITH CTOD(THISFORM.MTH_LIST.DISPLAYVALUE+'/'+B04.F02)
                         REPL F04 WITH B04.F01
                         REPL F05 WITH (B04.F04+B04.F13)*(-1)
                         THISFORM.PAGEFRAME1.PAGE1.PASS_CND.CAPTION='出貨紀錄'                         
                         SELE C10         &&出貨日報表
                         SET ORDER TO 5
                         SEEK B04.F07+B04.F01+B04.F03
                         IF FOUND()
                            DELE
                         ENDIF   
                         THISFORM.PAGEFRAME1.PAGE1.PASS_CND.CAPTION='出貨日報表'                                                  
                      ...
                      IF LEN(ALLTRIM(B04_1.F20))=10  
                          ...
                          IF FOUND() 
		                    ...
		                    SET ORDER TO 1	 && F01                              
                                    ...
                                    ENDIF                 
                                    SELECT  &K25Rec     
                                    DELETE  FOR LEFT(ALLTRIM(F15),10)=ALLTRIM(B04_1.F01)
                                   THISFORM.PAGEFRAME1.PAGE1.PASS_CND.CAPTION='發票明細檔'       
                              ENDIF     
                         ENDIF                                                                                          
                      ENDIF   
   ...
   IF MESSAGEBOX('是否確認資料無誤可以轉入INVOICE?',4+32+256,'請確認') = 6
       IF SEEK(B04.F01,'C22')=.F.	               
          IF RLOCK('B04') 
			...
			REPL F05 WITH 'CD'+F02
  			REPL F06 WITH 'INVOICE'
		    ...
		    ENDIF
                ...
                IF SEEK(PO_NO,'C22')=.T.      &&如果編號重複
                     SELE A24
                     SEEK 'C022 '+SUBSTR(DTOS(DATE()),3,4)+PO_NO
                     IF FOUND()
                         DO WHILE F01+F02+F08='C022 '+SUBSTR(DTOS(DATE()),3,4)+PO_NO
                                RLOCK()
                                DELE
                                UNLOCK
                                SKIP
                     ...
                     ENDIF                          
                      SELE A09
                      SEEK 'C022 '+SUBSTR(DTOS(DATE()),3,4)
                      IF FOUND()
                          RLOCK()
     		          HX=VAL(RIGHT(F08,4))+1
   	                  PO_NO=LEFT(F08,6)+PADL(ALLTRIM(STR(HX)),4,'0')
               ...
               REPL F12 WITH ''
               REPL F13 WITH DTOC(DATE())+' '+IIF(SEEK(SYS_OPER,'A01'),A01.F03,'')
               SELECT  B04
               SELECT F01,F03,F04,F06,F07,F14,F15 FROM B04 WHERE F01=ALLTRIM(THISFORM.PAGEFRAME1.PAGE1.TXTF01.VALUE)  INTO CURSOR B04A              
               SELECT B04A
	           GO TOP
	           DO WHILE !EOF()
	              SELECT C23	
             ...
             SELE C23
             USE
             =MESSAGEBOX('轉入成功,INVOICE單號'+PO_NO,0+64,'請至C22 INVOICE再次確認')         
                THISFORM.GRID1.SETFOCUS
                THISFORM.TRN_BOTT.ENABLED=.F. 
          ELSE 
             DO FILE_IMPACT
...
DEFINE CLASS TXTF03 AS TEXTBOX
  ...
  PROCEDURE DBLCLICK 
         ...
         IF THISFORM.SHRGROUP.VISIBLE=.T.  AND THIS.ReadOnly =.F.
              ...
              SELECT 0,F02,F01,F03,F04,F05,F09 FROM C05  WHERE ALLTRIM(F01) <> '備料紀錄' AND F06=THISFORM.PAGEFRAME1.PAGE1.TXTF06.VALUE;
               ...
               IF _TALLY>0
                    ...
                    DO CASE
	                   ...
	                   CASE NO>=1 AND CHK_IN='1'
			              ...
			              GO TOP
			              DO WHILE !EOF()
			                    IF  SEEK(THISFORM.PAGEFRAME1.PAGE1.TXTF01.VALUE+OQT_TEMP.F02+OQT_TEMP.F01,'B04')=.F.  AND IIF(SEEK(OQT_TEMP.F02,'B01'),.T.,.F.)
			                         SELECT  B04
				                 APPEND BLANK
				                 REPLACE F01 WITH THISFORM.PAGEFRAME1.PAGE1.TXTF01.VALUE
				                 REPLACE F02 WITH PADL(ALLTRIM(THISFORM.PAGEFRAME1.PAGE1.TXTF02.VALUE),2,'0')
				                 ...
				                 REPLACE F05 WITH IIF(SEEK(B04.F03,'B01'),IIF(B01.F97 = 'Y',INT_194,INT_193),INT_193)
				                 REPLACE F06 WITH THISFORM.PAGEFRAME1.PAGE1.TXTF06.VALUE
				                 ...
				                 REPLACE F16 WITH IIF(SEEK(B04.F14,'C00'),C00.F02,1)                  
      ...
      IF AT('?',ALLTRIM(THIS.VALUE))<>0
          ...
          DO CASE
                CASE AT('?',ALLTRIM(THIS.VALUE))>1 .AND. EMPTY(ALLTRIM(THISFORM.PAGEFRAME1.PAGE2.TXTF07.VALUE))                   
                          SELECT 0,F02,F01,F03,F04,F05,F09 FROM C05 WHERE F06=THISFORM.PAGEFRAME1.PAGE1.TXTF06.VALUE; 
                          AND LEFT(F02,AT('?',THIS.VALUE)-1)=LEFT(THIS.VALUE,AT('?',THIS.VALUE)-1);    
                          AND (C05.F04-C05.F05)>0 ORDER BY F02,F03 INTO CURSOR OQT READWRITE 
                CASE AT('?',ALLTRIM(THIS.VALUE))>1 .AND. !EMPTY(ALLTRIM(THISFORM.PAGEFRAME1.PAGE2.TXTF07.VALUE))                   
                           SELECT 0,F02,F01,F03,F04,F05,F09 FROM C05 WHERE F06=THISFORM.PAGEFRAME1.PAGE1.TXTF06.VALUE; 
                           AND LEFT(F02,AT('?',THIS.VALUE)-1)=LEFT(THIS.VALUE,AT('?',THIS.VALUE)-1) AND (C05.F04-C05.F05)>0;    
                           AND F01=THISFORM.PAGEFRAME1.PAGE2.TXTF07.VALUE ORDER BY F02,F03 INTO CURSOR OQT  READWRITE            
                CASE AT('?',ALLTRIM(THIS.VALUE))=1 .AND. EMPTY(ALLTRIM(THISFORM.PAGEFRAME1.PAGE2.TXTF07.VALUE))                   
                           SELECT 0,F02,F01,F03,F04,F05,F09 FROM C05  WHERE F06=THISFORM.PAGEFRAME1.PAGE1.TXTF06.VALUE;
                           AND (C05.F04-C05.F05)>0 ORDER BY F02,F03 INTO CURSOR OQT  READWRITE
                CASE AT('?',ALLTRIM(THIS.VALUE))=1 .AND. !EMPTY(ALLTRIM(THISFORM.PAGEFRAME1.PAGE2.TXTF07.VALUE))                   
                           SELECT 0,F02,F01,F03,F04,F05,F09 FROM C05  WHERE F06=THISFORM.PAGEFRAME1.PAGE1.TXTF06.VALUE;
                           AND (C05.F04-C05.F05)>0 AND F01=THISFORM.PAGEFRAME1.PAGE2.TXTF07.VALUE ORDER BY F02,F03 INTO CURSOR OQT  READWRITE              
          ENDCASE 
          SELECT OQT 
          ALTER TABLE OQT ADD C04_F15  N(11)
          ALTER TABLE OQT ADD C04_F16  N(8)
...
*!*	  AUTOCENTER=.T.
  ...
  ADD OBJECT CMND4 AS COMMANDBUTTON WITH;
   ...
   PROCEDURE INIT 
         ...
         ENDIF
         THISFORM.GRID1.COLUMN4.HEADER1.CAPTION='出貨日期'
